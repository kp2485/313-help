// Words, from strings/en.json and strings/es.json (docs/05 "Language").
//
// There is one source of truth: the build copies those two files out of the repository into the assets
...
//
// More languages are coming, including Arabic, which is written right to left. Nothing here assumes a direction,
// and no layout in this app uses left or right: every layout is written in start/end terms, and the manifest
// declares supportsRtl. Adding a language is a new strings/<lang>.json plus one line in LANGUAGES below.
...
object L {
    ...
    fun locale(): Locale = if (lang == "es") Locale("es", "US") else Locale.US
