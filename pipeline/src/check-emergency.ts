...
// Three-digit codes (911, 988, 211) are national and are not checked. Nothing here ever rewrites a number.

import { p, parsePhone, readCsv, today, writeCsv, type CsvRow } from './util.js';
import { fetchPage, phoneOnPage, type PageResult } from './page-match.js';
...
export function checkEmergencyRow(r: CsvRow, page: PageResult | null, day: string): EmergencyCheck {
  ...
  if (!ph || ph.number.length === 3 || !r.source_url || !page) return 'skipped';
  if (!page.ok) return 'unreadable';
  if (phoneOnPage(page.html, r.number!)) { r.verified_published_on = day; r.mismatch_on = ''; return 'match'; }
  r.mismatch_on = day;
  return 'mismatch';
}
...
if ((process.argv[1] ?? '').split('\\').join('/').endsWith('/src/check-emergency.ts')) {
  ...
  for (const r of rows) {
    const ph = parsePhone(r.number ?? '');
    const page = ph && ph.number.length !== 3 && r.source_url ? await fetchPage(r.source_url) : null;
