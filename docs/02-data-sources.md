...
| City of Detroit Food Pantry Finder | ArcGIS layer `food_bank_finder_data` behind the City's "Food Pantry Finder" app: 97 Detroit pantries and hot-meal sites with address, phone, hours in words and a coordinate | A (layer) / facts are the hosts' | `city_food_map` in `data/sources.yaml`, **staged**: rows are candidates, never published on their own. Licence unstated | Nightly re-read; a change opens a PR | **High** — last edited 2025-11-21 | Found 2026-09-23. 62 already listed, 17 new, 12 held leads, 6 places listed for something else; 35 of ours get a pattern the City states. [research/2026-09-23-city-food-map.md](research/2026-09-23-city-food-map.md) |
...
### Federal datasets for the wider service area (added 2026-09-24)

`pnpm ingest:federal` (`pipeline/src/ingest-federal.ts`, by hand, not nightly) reads four federal datasets and
keeps only rows inside a place of the service area (`placeAt`, `data/ingested/region.json`) and **not** in Detroit,
Hamtramck, Highland Park or Dearborn, which have their own sources. Every row is a **staged candidate** in
`data/staging/federal_<layer>.csv`, never a listing: each gets the same one-time entry check against the place's own
page before `pnpm import:lines` + `pnpm check:sources` can publish it. Ids are keyed to each publisher's permanent
reference (`ingest-ids.ts`), and anything a rule leaves out is counted, and named, on every run.

| Source | What | Tier | Licence | Notes |
|---|---|---|---|---|
| USGS The National Map, National Structures Dataset (`carto.nationalmap.gov/.../structures/MapServer`, layers 16 and 18) | Fire/EMS stations and police stations, with a point, street address and a per-row load date. **No phone** | A (staged) → `federal_fire.csv` (`safe.fire`), `federal_police.csv` (`safe.police`) | Public domain; the service describes itself as "public domain structures data" | One feature code per layer and no administrative type, so a "Headquarters" with no station number is **kept and flagged** in `note`; airport, military, training, marine and mounted units are dropped by name. Sheriff, State Police and campus police are flagged "not a city police department" |
| CMS, Hospital General Information (data.cms.gov provider data, `xubh-q36u`) | Michigan hospitals with `Emergency Services = Yes`, with the hospital's phone | A (staged) → `federal_er.csv` (`health.er`) | Public domain (federal work); no licence field in the metadata | **The file has no coordinate.** A third-party geocode is not allowed, so a row is placed by its mailing city **and** its stated county against region.json, carries `placed_by=city` and no lat/lon. A mailing city is not a municipality (a "Rochester" address may be in Rochester Hills); the research step settles that |
| IMLS, Public Libraries Survey, FY 2024 outlet file (`pls_fy2024_csv.zip`, June 2026) | Central libraries and branches, with the survey's own geocode and phone | A (staged) → `federal_library.csv` (`rec.library`) | Public domain (federal work) | Bookmobiles, books-by-mail and outlets the survey records as closed or temporarily closed (STATSTRU 03, 23) are left out; a library that did not answer that year (STATSTRU 25, facts carried forward) is flagged. Keyed by FSCSKEY + FSCS_SEQ, because LIBID repeats in the FY 2024 file. Latin-1 text. The year is a constant to bump when FY 2025 appears |
| HRSA, Health Center Service Delivery and Look-Alike Sites (data.hrsa.gov download) | Active health center sites (FQHCs and look-alikes) with phone and website | A (staged) → `federal_clinic.csv` (`health.clinic`) | Public domain (federal work) | Left out: mobile vans, administrative-only sites, correctional or carceral settings, and **every site in a domestic-violence setting** — HRSA already suppresses its street, and this repository carries no address or point for a DV organization; only the count is logged. School-based sites are flagged "may serve only students". The file is regenerated daily, so its date is the file's, not a record's. Members of Congress in the file are never read |
...
2. **Open data ingesters** (all read-only; output is committed, so a change shows up as a git diff):
   ...
   - `pnpm ingest:region` (by hand, first): the service area — every Census county subdivision a SMART or DDOT stop is in or a route runs through — from SMART's GTFS feed, the committed DDOT layers, TIGERweb county subdivisions and SEMCOG's community codes, into `data/ingested/region.json`. Every other ingest reads it through `pipeline/src/region.ts`.
   ...
   - `pnpm ingest:federal` (by hand): USGS fire and police stations, CMS emergency departments, IMLS library outlets and HRSA health center sites for the wider service area outside the four cities — staged candidates only (above, "Federal datasets for the wider service area").
   Detroit's precincts and fire stations are ingested by `pnpm ingest:safe`; Detroit's library branches are not ingested yet.
