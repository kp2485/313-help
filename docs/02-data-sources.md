...
| City calendar (detroitmi.gov/Calendar-and-Events) | ~70 upcoming events | B | **No RSS, iCal, or JSON feed exists** (every candidate URL is 404 or blocked). The pipeline reads the public listing pages once a day, spaced out, and keeps facts only: title, date, time, department, link. If fewer than 5 parse, the last good file is kept. Ask the City for a feed. Since about 2026-09-19 detroitmi.gov answers 403 to scripted requests, so events stay at the last good file (DECISIONS) |
