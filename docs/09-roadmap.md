...
| 5 | **iOS** — SwiftUI shell reading the same bundle. | 2026-09-20: `DetroitQuery` passes the 181 shared fixture cases in CI; the SwiftUI screens compile and run in the simulator, with reports, saved places, About/privacy, quick exit, an app-switcher shield and a release gate. `swift test` runs 73 tests in three targets, `HelpCore`'s 62 among them, all of them in CI. Never run on a real device and never signed for one. |
...
- **Holiday hours a place actually keeps.** Only the honest half is possible today: on a holiday a schedule-derived "open" is the `holiday` status — the usual hours, and "Call first" — in all three clients, from a date rule computed on the device (`schema/query-spec.md` "Holidays", 40 of the 181 fixture cases, DECISIONS 2026-09-20). At noon on Christmas Day 100 of 183 scheduled rows said "Open" before; none do now. What stays open is knowing anything **real**: **no source we read gives us any place's holiday hours.** A steward who learns some has two tools and no third — the `open_holidays` flag (open as usual) or an alert on that one row. Asking the largest pantries and clinics for their holiday schedule is worth a pass before December.
- **Convert the rest of the source-text tests to behavioural ones.** `apps/web/test/behaviour.test.ts` (36 tests) now drives the full-screen overlay, focus restoration, the private and shareable predicates, the link-scheme allow-list, the `_headers` file, the overdose note and the two entry-check badges in all four languages. About **64 of the 114 tests in `apps/web/test/web.test.ts` still read a source file and assert on its text.** Convert them as you touch them: a grep for a source line is green when the line is dead and red when it is harmlessly rewritten (DECISIONS 2026-09-20).
- **One line on the About screen saying this is an independent project.** The app now draws and names DDOT, SMART, Health Department and SEMCOG/Michigan State Police layers and says nowhere a resident can see that it is not theirs; the old "Not an official City of Detroit app" line was removed on 2026-09-20 and no store listing exists yet. A recommendation for Kyle, not a reversal (DECISIONS 2026-09-20).
- **Finish putting the native clients' privacy logic where CI can see it.** `HelpCore` (62 tests) and Android's `:core` now hold the install key and its daily hash, the outbox, the saved rules, the one session, the signature check and the release rules, and CI runs both; Android's HTTP code moved into `:core` in the same pass. Two areas still escape: the Swift signature maths and the backup-flag test sit inside `#if canImport(CryptoKit)` / `#if canImport(Darwin)`, so on the Linux CI container they pass while asserting nothing. Everything with an `android.` import or a SwiftUI import is still checked by hand or not at all.
- **A rollback floor for a fresh install on the web and Android.** The iPhone has one: a Release build verifies the snapshot it ships and embeds that snapshot's own `generated_at` as a floor below which no bundle is ever accepted. The web app and Android still only refuse a bundle **older than the one they already hold**, so a phone that has just installed has no floor at all and would take any validly signed bundle, however old. Key revocation is unbuilt everywhere (**Later**).
- **Take the `_headers` file out of theory.** The `/*` block is written — CSP with `frame-ancestors 'none'`, `Referrer-Policy: no-referrer`, `X-Content-Type-Options`, `X-Frame-Options` and a `Permissions-Policy` — but **Cloudflare Pages is what applies it and no Pages site exists**, so those headers have never been served to anybody. `index.html`'s `<meta>` copy is the fallback and cannot carry `frame-ancestors` at all. Create the Pages site (Kyle), then check the headers from outside with `pnpm smoke`.
- **The iPhone's release enforcement is not in version control.** `preflight.sh` and `verify-snapshot.swift` are committed, but the build phases that run them live in `apps/ios/Xcode/`, which is git-ignored. Anyone who recreates the project from the README has to add them back, and nothing fails if they do not. Decide whether `apps/ios/Xcode/` stops being ignored.
...
- **iPhone parity with the web app**: the street map, neighborhood pages, transit, add-a-place, archived listings in search, a listing's own alerts, an in-app language switch, a typed ZIP, parks, and the bundle-age banner on every list rather than only Home and Saved. (Quick exit and an app-switcher shield are done — the shield covers the whole window whenever the scene is inactive, which is what the platform offers, and quick exit is per screen.)
...
- **SEMCOG and the Michigan State Police** about the crash layer behind "Safe streets". SEMCOG's terms are **stated**, not unstated: its portal-wide [Copyright License Agreement](https://maps-semcog.opendata.arcgis.com/pages/copyright-license-agreement) grants a perpetual, royalty-free licence to reproduce and publish, **requires** the notice "Copyright © \<year\> SEMCOG. All Rights Reserved. Reproduction or Use Without Permission is Prohibited." (carried in NOTICE, and since 2026-09-20 printed on the Safe streets panel itself — as an app constant with the year written in, which whoever next refreshes the layer should check against the bundle's own copy), contains a **one-way indemnification clause**, and grants no third-party rights. So the question to SEMCOG is about the indemnification clause and attribution wording, and the question to MSP is its position on records that are theirs. **First, though, this is Kyle's own decision: accept the agreement including the indemnification clause, knowingly, or remove the layer** (DECISIONS 2026-09-20, "SEMCOG's terms were described wrongly"). **If SEMCOG or MSP objects, delete `data/ingested/crashes.json` and rebuild — the panel disappears on its own.**
...
- **A rollback floor and a key-revocation path.** A phone refuses a bundle older than the one it holds, but nothing revokes a pinned public key and nothing sets a version below which no bundle is ever accepted again. Both matter if a signing key is ever lost or stolen, and both need designing before they are needed, not after.
...
- **644 tests** in `pnpm test` on 2026-09-20 (query 181, api 91, pipeline 192, web 180), plus 73 Swift tests in three targets (`DetroitQueryTests` 5, `HelpCoreTests` 62, `AppParityTests` 6) and 72 Kotlin tests (`:query` 11, `:core` 61). The **181 fixture cases** in `schema/fixtures/` are run by all three implementations; the TypeScript suite emits one test per case, which is why its number and the fixture count are the same.
- **248 decisions recorded, 26 still open** (24 Open, 2 Proposed — [DECISIONS.md](DECISIONS.md)).
- **850 keys** in each of the four strings files.
