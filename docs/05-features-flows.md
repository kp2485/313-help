...
- **Neighborhoods** (2026-09-22): public numbers about each of the City's 205 neighborhoods (docs/13). **The tab IS a map** (Kyle, 2026-09-22: *"The Neighborhood home should be a map view by default showing the full screen map zoomed into the polygon of the neighborhood that the user is in, with a list option up in the top right for mobile users and next to the map for laptop web users"*; DECISIONS 2026-09-22; audit §3).
  ...
  - **Opening one.** An outline tapped on the map, or a row in the list, is an ordinary navigation to that area's own `#/n/<id>`, so a refresh lands on the same state. The page then **wears the map as a strip** across the top (38 % of the viewport), with a Back button and the area's name at its top start; tapping another outline in the strip swaps the page under it; Escape and the browser's Back do what the button does. As the page is read **down** the strip collapses to a 48 px bar that keeps Back and the name; **scrolling back up does not bring the map back — only the top of the page does** (Kyle, 2026-09-23). On iPhone the bar is the system's own navigation bar, so the strip collapses away entirely and there is one Back, not two. The collapse is height only — the map stays in the page and in the accessibility tree, the scroll position is never moved by us, and under Reduce Motion it happens without travelling. At ≥ 64 rem there is no strip: the map keeps its column and the area page takes the column the list was in.
  ...
  - **On iPhone (2026-09-22): the tab is a map there too.** The same landing on the polygon a person is standing in, the same 8 % margin and 4 m-per-point floor, the same Map | List switch in the map's top corner (mirrored to the leading corner in Arabic) with the map the default every launch, and the same strip — 38 % of the screen, a Back button and the name at its top leading corner, collapsing to a 48-point bar as the page is read and returning the moment the scroll reverses. The rules are `HelpCore/AreasHome.swift`, ported case for case from `apps/web/test/areas-map.test.ts`; the scroll is read with `onScrollGeometryChange` on iOS 18 and a `GeometryReader` inside the scroll view on iOS 17. Fixed in the same change: `Indicators.init(from:)` had never decoded the city-page keys, so on iPhone the four city pages had never existed at all.
