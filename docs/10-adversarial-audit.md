...
> **Status (2026-09-19).** The findings below are the audit as written; they are not rewritten. Checked against the code on 2026-09-19:
>
> - **Resolved:** A3 (the phone computes badges and bundle age), A4 (per-target nonce), A5 signing (Ed25519, two pinned keys; 911/988 hardcoded), A7 (overdose entry split), A8 (sensitive screens leave no URL; quick exit; DV and mental-health listings can't be saved), A9 (the City's published number), B2 (one ranking rule), B3 (overnight, DST and cancellation fixtures), B6 masking of report notes before storage (broader masking of steward notes and HSDS contacts is still pending), B8 (superseded by the app's own canvas street map), B11 (archived rows export as `defunct`), B13 (superseded by Urgent help in the top bar and the Help tab), C1, C2 (restore returns a listing to active), C3 (`strings/en.json` exists), C5 (repo is `313-help`), C14 (09 now follows CLAUDE.md's order).
> - **Still open:** A1.3 Turnstile or app attestation, A1.5 cap on community confirms, A1.6 QR site tokens, the A5 two-person rule for the emergency file, the A6 steward-load number on the admin page, B4 `signals.json`, B9 proposal near-duplicate collapse and a required consent field, B12 per-row source license in the dataset, C6 install prompt, C7 fetching archives older than 90 days on demand, C9 structured filters, C10 the youth route, C11 the first-launch line.
...
| C5 | 06's repo name didn't match the actual repo, which was named after DHD's product. "D Compassion" is DHD's product name — fine for a private repo, not for a public one (01: "not confusable with a city product"). | Rename before the repo goes public. |
