...
- **Validate**: HSDS 3.2 JSON Schema + our own checks (the service area's bbox, phone format, known category, schedule parse). A schedule whose `until` is past on an active row is a warning, not an error.
