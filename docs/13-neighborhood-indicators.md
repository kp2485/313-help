...
- `pipeline/src/ingest-crashes.ts` (`pnpm ingest:crashes`) reads SEMCOG's crash layer once a year, by hand, and writes counts per neighborhood and year into `data/ingested/crashes.json`. It is **not** in the nightly job: the layer gains a year at a time, and the licence is unsettled (docs/OPERATIONS, DECISIONS 2026-09-20).
