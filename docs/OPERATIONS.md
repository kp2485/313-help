...
| When SMART changes the communities it serves (and at least every quarter), **by hand** | `pnpm ingest:region` — the service area, worked out from where SMART and DDOT stop and run (DECISIONS 2026-09-24). If the list of places changed, run in this order and read each diff: `ingest:basemap`, `ingest:city zips`, `ingest:cities`, `ingest:mymap`, then research the new place's police line from its own page into `data/seed/emergency.csv` (`area` = its id) and run `pnpm check:emergency`. A new place outside `SERVICE_BBOX` fails a test: widen the box in `packages/query/src/areas.ts`, `Locate.swift` and `Locate.kt` together | 30 minutes |
| Once, then when a site changes | The police lines for **Detroit, Shelby Township and Wixom** are missing: their sites answer scripts with 403. Read each police department's own page in a browser, add the row with its `area`, and record the date in `verified_published_on` | 10 minutes |
...
| When a site starts refusing our fetcher | Add a row to **`data/seed/script-refusing-hosts.csv`**: the host (no `www.`), `refusing_since` (the day it started, ISO), what the refusal looks like ("HTTP 403 with a Cloudflare challenge page"), the day you checked, and a note. This is not paperwork: from that date the build **refuses** to let any `active` row on that host claim `entry_method=auto_check`, because a badge saying "a program matched this to their website" would be false, and `pnpm check:emergency` stops fetching emergency numbers on that host and logs them for a person (unread, never a mismatch). A page that answers 200 but draws its content with a script counts too. Four hosts today — `detroitmi.gov` and `dmc.org` (2026-09-20), `cskdetroit.org` (2026-09-19), `lathrupvillage.org` (2026-09-24, a script-drawn shell). Rows on that host are read in a browser instead and set `entry_method=web`. If a site starts answering scripts again, leave the row and note the date it recovered rather than deleting it | 2 minutes |
