...
Source: reverse-engineering of `com.detroithealthdepartment.dcompassion` v1.0 (built 2024-12-10, .NET MAUI Blazor Hybrid, Radzen UI, Android only). The directory content is public program information; DHD staff are informally supportive, and a written authorization letter is pending (see 08).
...
| Services directory (~18 DHD programs, phone + website) | Hardcoded strings in the DLL | **Keep the content**, move it to a DHD-owned sheet |
...
| No data ownership model | Directory could only change with a code release | Owner feeds (DHD sheet), stewards, HSDS dataset (03, 04) |
...
| App store submission never happened | Nobody at the city could complete it | Shipped by an individual developer with an authorization letter (08) |
