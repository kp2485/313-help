...
| 09 | [Roadmap & open questions](09-roadmap.md) | The build order that got us here, then everything left by horizon (Now · Next · Later) with who each item waits on, and the decisions Kyle needs to make. The short version is in [/README.md](../README.md) |
...
| — | [/README.md](../README.md) | The product's front page: what it is, what is in it today, the privacy promises, the roadmap by horizon, and how to help. Written for people who are not going to read these docs |
...
- 2026-09-20 (evening): **a front page for the repository, the Transit app link, and the Android app's first compile and emulator run.**
  - *A root `README.md` at last.* GitHub's home page for this repository showed nothing until now. It is the product's front page — what the app is, what is in it today with every number re-derived from a build, the privacy promises in one list, a roadmap by horizon (Now → launch, Next → three months, Later), a quick start, how to help, and the data licences including the ones still flagged. [docs/09](09-roadmap.md) is now the detailed half of that roadmap.
  - *Transit link-out:* **Bus directions in the Transit app** on a listing, on web, iPhone and Android — Transit's own documented `transit://directions?to=…` scheme, destination only, **never a `from`** and never an origin or identifier, hidden where directions are withheld (DV and crisis listings) and on a phone with no Transit app. Nothing is fetched from their servers; the plain word "Transit" in text, no logo. Their linking terms are still an open question (DECISIONS).
  - *Android:* the `:app` half **compiled, its tests ran, it assembled a 1.13 MiB debug APK and it ran** — on an AOSP API 35 emulator, never a phone. Five real errors were found and fixed; two of them would have crashed every phone from `minSdk` 24 to Android 14. The APK carries two permissions and no third-party code at all. The CI job was fixed in the same pass: an ubuntu runner ships a licensed Android SDK, so the job had quietly started pulling `:app` in; it now sets `HELP313_NO_ANDROID=1` and builds `:query` and `:core` only, **on purpose** — a workflow should not accept Google's SDK licence on this repository's behalf.
  - *The importer reports collisions:* `pnpm import:lines` used to drop a line silently when its `sal_id` already existed — which is how the one Highland Park tax-help line went missing from a 25-line file. It now prints `skipped: id collision with <sal_id>` and counts it in the summary, and a line that corrects an address for the same organisation and phone prints an informational note. Ids themselves are unchanged.
  - *Arabic and Bengali, walked screen by screen:* "am"/"pm" became translated strings (`clock.am`, `clock.pm`) while digits stay Western; dollar amounts are written the way the sale record writes them (`$85,000`), falling back to `en-US` where a language's own `Intl` rules do not lead with the sign; the list separator is a string (`list.sep`); and the uppercase, letter-spaced section headings now apply only under `:lang(en)`/`:lang(es)`, because letter-spacing breaks Arabic joining and Bengali conjuncts. The iPhone and Android apps still hard-code am/pm — a roadmap item. **Searching in Arabic or Bengali returns nothing today**, because the listings are written in English; the empty state needs a one-line hint, worded with the native reviewer.
  - *Numbers re-derived:* the bundle holds **531 listings**, not 528, and `pnpm test` is **503** (query 111, api 80, pipeline 179, web 133). The main script is 72.8 KB gzipped plus 6.6 KB of CSS. Both figures below were written before the last rows and tests landed; the corrections are here rather than in the entry underneath.
...
- 2026-09-20 (coordinated build): **528 listings, four tabs with one Map, an accessibility pass, an Android client written, and the deploy blockers cleared down to Kyle's own steps.**
  - *Data:* **528 listings**, up from 418 *(531 by the end of the day — see the entry above)*. Every category in docs/03 now has at least one live listing except warming and cooling centers, which are alert-driven and empty on purpose. Two new kinds of help, `health.er` (emergency room) and `health.urgent` (urgent care) — they cannot be `health.clinic`, which means free or low-cost, and none of these places says it is. 26 Wayne County naloxone and test-strip stations arrive through a new reader for Google My Maps KML (`pnpm ingest:mymap`); they publish a city and a point and **no street address**, which the pipeline, the bundle, the web app and the iPhone app all now handle without inventing one. 24 Health Department programs are live at last.
  ...
  - *Checks:* `pnpm test` is **487** (query 111, api 80, pipeline 172, web 124), plus 111 Swift fixture cases and 28 iOS app tests. *(503 by the end of that evening; see the entry above.)* Differences for a steward are in **[CHECKS-2026-09-20.md](CHECKS-2026-09-20.md)**.
  - *Languages:* **Arabic and Bengali are built** — four languages in all (English · Español · العربية · বাংলা), each a lazily loaded chunk, each named in its own words in the switch. Arabic mirrors the whole interface (right to left) on web, iPhone and Android. **Both were drafted by machine and no native speaker has read either one yet**, so DECISIONS keeps them Open: crisis, overdose and domestic-violence screens are the first thing a reviewer should read. Nothing in the app claims they were checked. Every digit in both is Western (0-9), so a number reads as it is dialled (DECISIONS 2026-09-20).
